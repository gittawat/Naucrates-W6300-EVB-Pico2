...
template <typename... Modules>
    requires(ModuleConcept<Modules> && ...)
class StaticModuleRunner
{
public:
    explicit constexpr StaticModuleRunner(int32_t frequency_hz, Modules&&... modules)
        : frequency_hz_(frequency_hz)
        , interval_us_(frequency_hz > 0
              ? (1'000'000 / static_cast<uint32_t>(frequency_hz))
              : 1000)
        , next_deadline_(time_us_32() + interval_us_)
        , modules_(std::tuple<Modules...>(std::move(modules)...))
    {
    }

    void configure()
        requires(ConfigurableModule<Modules> && ...)
    {
        std::apply([](auto&... mod) {
            (mod.configure(), ...);
        }, modules_);
