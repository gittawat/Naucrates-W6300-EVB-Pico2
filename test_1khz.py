#!/usr/bin/env python3
"""1 kHz UDP echo stress test for W6300-EVB-Pico2."""
...
print(f"Sending at {RATE} Hz for {DURATION}s to {HOST}:{PORT}...")

while time.monotonic() < deadline:
    now = time.monotonic()
    if now >= next_send:
        payload[:] = struct.pack('<IQ', sent, int(now * 1_000_000))
        sock.send(payload)
        sent += 1
        next_send += INTERVAL
